"""
`iot_data_audit` is the top-level command for accessing various subcommands.
...
def _try_import(module):
    ...
    return importlib.import_module(relative_module, 'iot_data_audit.bin')
...
def _build_commands():
    ...
    if unavailable:
        ...
        for module, ex in unavailable:
            DESCRIPTION += (
                f'\nWARNING: "iot_data_audit {module}" is unavailable due to:'
...
def main():
    ...
    top_parser.add_argument(
        '--version', '-V',
        action='version',
        version=iot_data_audit.__version__,
        help="Show the iot_data_audit version number and exit."
    )
    ...
    logger = logging.getLogger('iot_data_audit')
